...
export default function Portfolioo() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Ensure this URL points to your deployed Vercel backend
            const response = await fetch('https://portfolio-mimi-rho.vercel.app/api/contact', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (response.ok) {
                setStatus({ success: true, message: 'Message sent successfully! 🚀' }); // Typo Fixed
                setFormData({ name: '', email: '', message: '' }); // Clear Form
            } else {
                setStatus({ success: false, message: data.error || 'Something went wrong.' });
            }
    ...
    // Merged & Optimized Single Intersection Observer
    useEffect(() => {
        const observerOptions = {
            root: null,
            rootMargin: '0px',
            threshold: 0.15, // 15% section visible hone par trigger hoga
        };
    ...
    // Added 'githubLink' properties so buttons render correctly
    const projects = [
        {
            title: 'Marrie Mart',
            category: 'Full-Stack',
